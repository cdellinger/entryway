...
var User = function(){
	...
	this._update = function(cb){
		...
		if (self.homeLocation !== undefined){
			updateExpression += ', homeLocation = :homeLocation';
			expressionAttributeValues[':homeLocation'] = self.homeLocation;
		}
		else{
			removeExpression += 'homeLocation, ';
		}

		if (self.fullName !== undefined){
			updateExpression += ', fullName = :fullName';
			expressionAttributeValues[':fullName'] = self.fullName;
		}
		else{
			removeExpression += 'fullName, ';
		}

		if (self.bio !== undefined){
			updateExpression += ', bio = :bio';
			expressionAttributeValues[':bio'] = self.bio;
		}
		else{
		...
		if (self.avatar !== undefined){
			...
			expressionAttributeValues[':avatar'] = self.avatar;
		}
		else{
